    ...
    <section className="w-full bg-white py-16 px-4 md:px-8">
      <div className="max-w-[1110px] mx-auto">
        ...
        <div className="flex items-center justify-between mb-8 border-b border-gray-100">
          <h2 className="font-sans text-4xl md:text-4xl font-light text-slate-900 mt-1 uppercase tracking-tight pb-4">
            Informasi & Event
          </h2>
            ...
            {["Informasi", "Event"].map((tab) => (
              <button
                ...
                className={`pb-4 px-1 text-xs md:text-sm font-normal transition-all relative ${
                  activeTab === tab ? "text-[#006adb]" : "text-gray-400"
                }`}
              >
                {tab.toUpperCase()}
                {activeTab === tab && (
                  <span className="absolute bottom-0 left-0 w-full h-[3px] bg-[#006adb]" />
                )}
              </button>
            ))}
          </div>
                    ...
                    {activeTab === "Informasi" ? (
                        ...
                        <div className="px-2 md:px-3 pb-3 md:pb-4 flex flex-col flex-grow font-sans">
                          ...
                          <div className="mt-auto">
                            <a
                              href={item.link || "#"}
                              target="_blank"
                              className="text-[11px] md:text-[13px] font-semibold text-[#006adb] hover:underline"
                            >
                              Selengkapnya...
                            </a>
                          </div>
                      ...
                      <div className="bg-white overflow-hidden border border-gray-100 flex flex-col group transition-all hover:shadow-md h-full">
                        ...
                        <div className="p-3 md:p-4 flex flex-col flex-grow">
                          ...
                          <h3 className="text-xs md:text-sm font-bold text-slate-900 mb-1 md:mb-2 leading-snug group-hover:text-[#006adb] transition-colors line-clamp-2">
                            {item.title}
                          </h3>
                          ...
                          <a
                            href={item.link || "#"}
                            target="_blank"
                            className="flex items-center gap-1 md:gap-2 text-gray-500 font-bold text-[10px] md:text-[12px] group-hover:text-[#006adb]"
                          >
